...
public class UpdateNote {
    ...
    public static void main(String[] args) {
        ...
        UpdateNoteOptions opts = new UpdateNoteOptions.Builder().accountId(accountId).providerId(testString)
                .shortDescription(testString).longDescription(testString).kind("FINDING").id(testString)
                .noteId(testString).reportedBy(reporter).finding(finding).build();

        Response<ApiNote> resp = service.updateNote(opts).execute();
